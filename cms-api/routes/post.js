...
module.exports = (_app) => {
	...
	_app.post('/post', CT.ensureAuthorized, (_req, _res) => {
		...
		const post = {
			"campaign_id": (_req.body.campaign_id)?_req.body.campaign_id:'',
		...
		if(post.campaign_id == '' || post.title == '' || post.track_id == '' || post.post_content == '' || post.org_id == ''){
			...
			db.executeSql("select id from posts where not status = -1 and track_id = '"+post.track_id+"'"+filter,(_cerr,_cdata) => {
				if(_cerr){
					...
					if(!_cdata && _cdata.length > 0) {
						...
						let msg = '';
						if(id == ''){
							sql = "INSERT INTO posts (`campaign_id`, `title`, `post_desc`, `posted_on`, `track_id`, `post_img_vid`, `post_type`, `post_content`, `remark`, `inserted_by`, `inserted_on`, `status`, `ip`, `org_id`) VALUES ('"+post.campaign_id+"', '"+post.title+"','"+post.post_desc+"', '"+post.posted_on+"', '"+post.track_id+"','"+post.post_img_vid+"','"+post.post_type+"','"+post.post_content+"','"+post.remark+"','"+post.inserted_by+"', now(), '"+post.status+"', '"+post.ip+"','"+post.org_id+"')";
							msg = "Inserted Successfully";
						}else{
							sql = "UPDATE posts SET title = '"+_req.body.title+"', org_id = '"+_req.body.org_id+"', campaign_id = '"+_req.body.campaign_id+"',post_desc = '"+_req.body.post_desc+"', posted_on = '"+_req.body.posted_on+"', post_type = '"+_req.body.post_type+"', post_content = '"+_req.body.post_content+"', remark = '"+_req.body.remark+"', updated_on = now(),track_id = '"+_req.body.track_id+"' WHERE id  = '"+id+"'";
